...
static iomux_v3_cfg_t const disp0_bkl_pads[] = {
	...
	IOMUX_PADS(PAD_SD1_DAT3__GPIO1_IO21	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)),
};
...
void board_enable_lvds(const struct display_info_t *di, int enable)
{
	if (enable == 1) {
		...
		mdelay(50);
		gpio_direction_output(IMX_GPIO_NR(1, 21), enable); /* DISP_PWM */
...
int board_late_init(void)
{
	...
	sprintf(board, "crix-c%s%1x00", cpu, ddr_model);

	printf("Board: %s\n", board);
	env_set("board_name", board);

	if (env_get_yesno("fdt_file") == -1){
		sprintf(fdt_file, "imx6%s-crix-c%sx00.dtb", cpu, cpu);
		env_set("fdt_file", fdt_file);
	}
	if (env_get_yesno("curt_config") == -1){
		sprintf(curt_config, "crix-c%sx00", cpu);
