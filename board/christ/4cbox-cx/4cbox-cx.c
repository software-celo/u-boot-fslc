/*
 * Copyright (C) 2016-2018 Christ Electronic Systems GmbH
...
static iomux_v3_cfg_t const gpio_pads[] = {
	IOMUX_PADS(PAD_CSI0_VSYNC__GPIO5_IO21	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //WIFI_DISABLE
	IOMUX_PADS(PAD_CSI0_DAT4__GPIO5_IO22	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //BT_DISABLE
	IOMUX_PADS(PAD_GPIO_16__GPIO7_IO11	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //MCB_ENABLE
	IOMUX_PADS(PAD_CSI0_DAT6__GPIO5_IO24	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //nFULL_CARD_POWER_OFF

	IOMUX_PADS(PAD_GPIO_2__GPIO1_IO02	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_USR_RED
	IOMUX_PADS(PAD_GPIO_3__GPIO1_IO03	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_USR_GREEN
	IOMUX_PADS(PAD_GPIO_4__GPIO1_IO04	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_USR_BLUE
	IOMUX_PADS(PAD_GPIO_17__GPIO7_IO12	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_ONLINE
	IOMUX_PADS(PAD_KEY_COL0__GPIO4_IO06	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_SIG1
	IOMUX_PADS(PAD_KEY_ROW0__GPIO4_IO07	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_SIG2
	IOMUX_PADS(PAD_KEY_ROW1__GPIO4_IO09	| MUX_PAD_CTRL(GPIO_PD_PAD_CTRL)), //LED_SIG3
};
...
{
/*
 * optimized pad skew values depends on CPU variant on the 4cbox board:
 * i.MX6Q or i.MX6DL
 */
...
int board_late_init(void)
{
	...
	sprintf(board, "4cbox-c%s%1x00", cpu, ddr_model);

	printf("Board: %s\n", board);
	env_set("board_name", board);

	if (env_get_yesno("fdt_file") == -1){
		sprintf(fdt_file, "imx6%s-4cbox-c%sx00.dtb", cpu, cpu);
		env_set("fdt_file", fdt_file);
	}
	if (env_get_yesno("curt_config") == -1){
		sprintf(curt_config, "4cbox-c%sx00", cpu);
